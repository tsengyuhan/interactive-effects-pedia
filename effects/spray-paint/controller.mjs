...
export function createController(root) {
  ...
  root.innerHTML = `<div class="controller-inner">
    <div class="eyebrow">INTERACTIA / POCKET STUDIO</div><h1>${t('掌心裡的噴漆罐')}</h1>
    ...
    <div class="palette"></div><label class="sensitivity">${t('轉向靈敏度')}<input type="range" min="0.5" max="2" step="0.1" value="1"></label>
    <p class="controller-footnote">${t('直立握住手機，左右轉向、上下抬低。不是平移追蹤。')}<br>${t('不使用相機或麥克風')}</p>
  </div>`;
  ...
  let permission = false, waitingAt = 0, pointer = null, startY = 0, audio = null, noise = null, gain = null, sounding = false;
  ...
  function sound(on) {
    ...
    sounding = on;
    if (gain && audio) gain.gain.setTargetAtTime(on ? .055 : 0, audio.currentTime, .018);
  ...
  function prepareAudio() {
    try {
      if (!audio) {
        ...
        noise = audio.createBufferSource(); noise.buffer = buffer; noise.loop = true;
        const filter = audio.createBiquadFilter(); filter.type = 'highpass'; filter.frequency.value = 1100;
  ...
  function send() {
    ...
    const pressed = canSpray(state, performance.now());
    sound(pressed);
  ...
  calibrate.addEventListener('click', () => {
    if (!frame || !state.paired || performance.now() - state.sensorAt > SENSOR_TTL) return;
  ...
  const changedDirection = () => uncalibrate('螢幕方向已改變，請重新確認對位。');
  ...
  nozzle.addEventListener('pointerdown', event => {
    if (!state.calibrated || !state.paired || pointer !== null || event.button !== 0) return;
    event.preventDefault(); prepareAudio(); pointer = event.pointerId; startY = event.clientY;
  ...
  nozzle.addEventListener('keydown', event => {
    ...
    event.preventDefault(); prepareAudio(); state.held = true; nozzle.style.transform = 'translateY(34px)';
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    if (document.hidden) { state.paired = false; audio?.suspend().catch(() => {}); }
  }, options);
  ...
  const timer = setInterval(() => {
    ...
    calibrate.disabled = !state.paired || !frame || now - state.sensorAt > SENSOR_TTL;
    sound(canSpray(state, now)); send();
  }, 40);
  connect();
  return () => {
    stop(); disposed = true; clearInterval(timer); events.abort(); relay?.close();
    noise?.stop(); audio?.close().catch(() => {});
  };
}
