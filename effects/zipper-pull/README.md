...
- 連續開拉鍊幀（WebP 含 alpha 開口），Canvas 2D 依進度交叉淡化。牛仔褲（7 幀）與包包（9 幀）的開口幀都是 AI 生成後，對齊閉合底圖、依開口顏色去背轉 alpha 而來
...
- **包包**：完全拉開後，開口內仍會出現一小塊不規則的黑色瑕疵（去背沒挖乾淨的殘留），另外拖曳時的拉鍊陰影（`rimShadow`，`effect.js` 的 `drawScene` 內以 `blur`+`brightness` 合成）太深太明顯，看起來不自然，需要調淡調小。
- **牛仔褲**：拉鍊頭圖示（`slider-jeans2.png`）沒有精準對準幀圖上實際的拉鍊齒條，兩者有橫向落差；另外拖曳超過一半進度時，畫面對應的幀仍是沒有挖孔的實心圖（看不到後方內容），需要重新檢查對應幀的 alpha 去背與 `apex` 對位。
