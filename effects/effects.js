...
window.EFFECTS = [
  ...
  },
  {
    ...
    instructions: "對著麥克風打鼓、拍手或發聲。貓咪煙火風格是純黑背景，隨機射出一隻貓（橘虎斑、賓士、布偶、暹羅），一路變大並逐格演出牠的專屬動作，到頂點時一邊繼續放大一邊化成細點狀的粒子雲、聚成更大的貓頭形狀再散開；紙張破裂煙火風格的背景是一張白紙，色鉛筆線從畫面底部一口氣甩上去畫出升起的軌跡（每發隨機抽色），到頂點時紙從線的末端被撕開、四周甩出幾筆彩色火花、邊上掀起紙片露出後面的全息流動動畫，撐一下再原樣倒放合回去；也可以選「只看背景動態」把煙火整個關掉，讓那層流動鋪滿畫面。可調視覺風格、觸發音量、煙火大小與顏色",
    ...
    principle: [
      ...
      "色鉛筆線是一記快甩：170 毫秒、ease-out 起手就最快，從畫面底緣外側起筆不留空白，一格拆四小段畫所以甩得快也不會斷成折線；每一發隨機抽色，破口周圍再甩出幾筆彩色短觸當綻放的火花",
