...
window.EFFECTS = [
  ...
  },
  {
    ...
    title: "爆炸頭",
    category: "身體動作",
    description: "把你的臉變成綁在路邊消防栓的立體氣球，打氣直到爆炸",
    instructions: "一個人正對鏡頭，左右移頭帶動氣球甩動；連點打氣，爆炸後按重置，風吹走碎片才生成新球。資訊面板可調背景、充氣速度、最大尺寸與魚眼程度。",
    tech: ["MediaPipe Image Segmenter", "MediaPipe Face Detector", "WebGL", "Canvas 2D", "Web Audio API", "getUserMedia"],
    principle: [
      "本地模型擷取即時五官，貼滿有曲面光影的不透明氣球；畫面只呈現路面、消防栓與氣球。",
      "浮力、風力與移頭慣性牽動多段繩索；固定綁點與繩長約束讓氣球自然甩動，打氣時彈性膨脹。",
      "近景街道共用地面透視，魚眼五官包覆立體球面並投影路面；碎片落地保留，重置時先由陣風吹走再生成新球。"
    ],
  ...
  },
  {
    ...
    title: "聲音煙火",
    category: "聲音互動",
    description: "對麥克風發聲或打鼓就發射一發煙火：寫實貓咪頭碎成貓形點狀雲，或一道色鉛筆線把白紙炸出一個洞；也能關掉煙火只看後面那層流動背景",
    instructions: "對著麥克風打鼓、拍手或發聲。貓咪煙火風格是純黑背景，隨機射出一隻貓（橘虎斑、賓士、布偶、暹羅），一路變大並逐格演出牠的專屬動作，到頂點時一邊繼續放大一邊化成細點狀的粒子雲、聚成更大的貓頭形狀再散開；紙張破裂煙火風格的背景是一張白紙，色鉛筆線從畫面底部一口氣甩上去畫出升起的軌跡（每發隨機抽色），到頂點時紙從線的末端被撕開、四周甩出幾筆彩色火花、邊上掀起紙片露出後面黑白流體潑漆＋全息虹光的動畫，撐一下再原樣倒放合回去；也可以選「只看背景動態」把煙火整個關掉，讓那層流體鋪滿畫面。可調視覺風格、觸發音量、煙火大小與顏色",
    tech: ["Web Audio API", "getUserMedia", "Canvas 2D", "WebGL", "GLSL", "逐格動畫"],
    principle: [
  ...
  },
  {
    ...
    instructions: "按住拉鍊頭沿拉鍊方向拖曳（牛仔褲上下、包包左右），拖曳時會有拉鍊音效；拉回閉合後，下次拉開會換內容。也可開啟攝影機，用拇指與食指捏合拉鍊頭拖曳",
    tech: ["Canvas 2D", "AI 連續幀", "Pointer Events", "Web Audio", "MediaPipe Hands", "getUserMedia", "GIF", "MJPEG"],
    principle: [
      "拉開過程用連續幀（布料皺褶真實），開口內部是 alpha 透明。牛仔褲 7 幀（第 1 幀起鈕扣鬆開，連腰口一起張開）、包包 9 幀，接進來前先逐幀對齊色彩，拖曳時才不會閃。程式在相鄰幀間交叉淡化，並沿拉鍊軸把較閉的那幀漸變補滿，開口的外緣一律收在拉鍊頭上；開口尖點與齒條位置都是逐幀量測後內插，拉鍊頭才會一直貼著齒條跟手移動",
