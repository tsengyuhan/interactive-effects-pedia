...
window.EFFECTS = [
  ...
  },
  {
    ...
    title: "創世紀手指",
    category: "身體動作",
    description: "對著鏡頭伸出手，畫面從對角對側伸出一隻手，越往中間靠近，最後兩隻食指指尖相觸，像《創世紀》那幅畫",
    instructions: "面對鏡頭伸出手；webcam 會去背、疊在純色背景上。當你的手越往畫面中央移動，對面就從「對角對側」伸出一隻手越靠近你，最後兩隻食指指尖相碰（你的手往上伸，對面的手就從上往下伸）。對面的手每次隨機是五種風格之一（文藝復興壁畫、卡通、貓掌、機器人、外星人）。畫面中越多手就出現越多對面的手。可調去背模式（只露出手／整個人）、手的大小、最多互動手的數量",
    tech: ["MediaPipe Hands", "MediaPipe Image Segmenter", "getUserMedia", "Canvas 2D"],
    principle: [
      "MediaPipe 手部模型即時取得每隻手的食指指尖座標，跨幀以最近鄰配對維持身分與隨機風格",
      "「只露出手」模式用手部關鍵點凸包＋手指膠囊組成「範圍框」把臉排除在外，框內再交集人像分割遮罩修出真實手緣（小魚際、指縫都貼合），兼顧自然輪廓與不漏臉；「整個人」模式直接用人像分割模型，兩者都疊在純色背景上",
      "以畫面中央為對稱點，對面的手永遠在使用者手的對角對側、朝中央伸來，靠指尖到中央的距離換算接近度，相觸於使用者指尖"
    ],
    requirements: ["攝影機", "建議 Chrome / Edge", "舊機器 fps 較低", "需經 start.bat 或 HTTPS 開啟"],
    offline: true,
    offlineNote: "",
