...
window.EFFECTS = [
  ...
  },
  {
    ...
    instructions: "對著麥克風打鼓、拍手或發聲。貓咪煙火風格是純黑背景，隨機射出一隻貓（橘虎斑、賓士、布偶、暹羅），一路變大並逐格演出牠的專屬動作，到頂點時整張臉碎成細點狀的粒子雲、定格成貓頭形狀再散開；紙張破裂煙火風格的背景是一張皺白紙，鉛筆線畫出煙火升起的軌跡，到頂點時紙被炸出一個洞、洞後面透出流動的循環動畫，接著紙慢慢蓋回去收成撕裂縫再淡掉。可調視覺風格、觸發音量、煙火大小與顏色",
    ...
    principle: [
      ...
      "紙張的破洞是程式畫的多邊形：只有部分邊撕出毛刺，兩端往外推成翻起的紙片，再靠內外陰影做出厚度；收合時沿短軸壓扁就成了撕裂縫"
    ],
