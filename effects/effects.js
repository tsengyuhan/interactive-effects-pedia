...
window.EFFECTS = [
  ...
  },
  {
    ...
    title: "聲音煙火",
    category: "聲音互動",
    description: "對麥克風發聲或打鼓就發射一發煙火：寫實貓咪頭碎成貓形點狀雲，或一道鉛筆線把白紙炸出一個洞",
    instructions: "對著麥克風打鼓、拍手或發聲。貓咪煙火風格是純黑背景，隨機射出一隻貓（橘虎斑、賓士、布偶、暹羅），一路變大並逐格演出牠的專屬動作，到頂點時整張臉碎成細點狀的粒子雲、定格成貓頭形狀再散開；紙張破裂煙火風格的背景是一張白紙，鉛筆線畫出煙火升起的軌跡，到頂點時紙從線的末端被撕開、邊上掀起紙片露出後面的全息流動動畫，撐一下再原樣倒放合回去。可調視覺風格、觸發音量、煙火大小與顏色",
    ...
    principle: [
      ...
      "上升位置用 smootherstep：慢起步、中段最快、到頂點前收住，做出衝上去再停住的加速度感",
      "整套模擬固定跑 12 fps，四隻寫實貓各有一套 6 格序列圖，依飛行進度換格做出逐格動畫的頓挫感",
      "爆炸從貓咪圖本身長出來：縮到 56×56 取樣換成帶原色的粒子，先定格成剪影、再全部聚到放大的貓頭輪廓上，最後才散成點狀雲",
      "紙張的破口改用序列圖播放：四種不規則撕法各 6 格從小裂口撕到滿，合回去就是同一組影格倒放；貼片只留掀起的紙片與影子，平坦紙面全透明才不會蓋掉鉛筆線",
      "洞裡那張全息流動圖是整片鋪滿畫面的，clip 出洞形後把變換重設回畫面座標再貼上，洞開合時後面的影像才不會跟著縮放"
    ],
