...
window.EFFECTS = [
  ...
  },
  {
    ...
    instructions: "面對鏡頭，伸出食指、收起中指與無名指，停住約半秒，指尖發光後就能在空中畫一個封閉形狀；路徑閉合後發光 1 秒，那塊畫面便掉落，碎片會互相碰撞彈跳、堆在畫面底部約 8 秒後淡出；洞裡透出套用特效的即時畫面。可連續切好幾刀，洞會累積，按右上角「重置世界」復原。可選洞內特效（科技像素／動態亂碼／印刷拼貼／色塊版畫），並調整特效顆粒、洞緣厚度與掉落速度",
    tech: ["MediaPipe Hands", "getUserMedia", "Canvas 2D", "Matter.js"],
    principle: [
      "MediaPipe 手部模型即時取得食指指尖座標，以指節距離判定「只伸食指」手勢，維持半秒確認後才開始記錄軌跡",
      "軌跡自我相交或首尾靠近即形成封閉多邊形，發光一秒後把該區畫面快照成碎片",
      "碎片以凸包剛體交給 Matter.js 模擬：依形狀翻倒、彈跳、堆疊，支撐消失自動掉落，落定 8 秒後淡出；洞是裁切窗口，洞緣取畫面色壓暗畫出牆體斷面與內陰影做出厚度，透出特效版即時畫面",
